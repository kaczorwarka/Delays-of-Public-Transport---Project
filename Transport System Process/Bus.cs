...
public class Bus : MeansOfTransport 
{
    //Pobranie atrybutow dla klasy Bus - Punkt w Scorecard: Atrybuty własne
    Parameters parameters = (Parameters)Attribute.GetCustomAttribute(typeof(Bus), typeof(Parameters));
    private string meansOfTransport = "Bus";
    ...
    public async Task FileReader()
    {
        ...
        await Task.Run(() =>
        {
            ...
            IEnumerable<string> Stops = filelist.getFileList(path).Where(S => S[0] != 'A');
    ...
    public void Sender(string CurrentStop, double WaitingTime, bool toEarly)
    {
        ...
        using (var writer = new StreamWriter(SystemData))
        {
            ...
            else
            {
                ...
                writer.WriteLine($"It's to early for this line, first bus will come at {parameters.StartHour_H}:{parameters.StartHour_Min}0");
                writer.Flush();
            }
        }
    }
