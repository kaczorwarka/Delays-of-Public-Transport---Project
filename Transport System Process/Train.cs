...
[Parameters(0.15, 0.75, 0.25, 1.2, 2, 1.0, 1.0, 8, 00)]

//Wykorzystanie Interface - Punkt w Scorecard: Kontrakty: Interface
public class Train : MeansOfTransport
{
    //Pobranie atrybutow dla klasy Bus - Punkt w Scorecard: Atrybuty własne
    Parameters parameters = (Parameters)Attribute.GetCustomAttribute(typeof(Train), typeof(Parameters));
    private string meansOfTransport = "Train";
    ...
    public async Task FileReader()
    {
        ...
        await Task.Run(() =>
        {
            ...
            IEnumerable<string> Lines = filelist.getFileList(path).Where(L => L[0] == 'P');
            IEnumerable<string> Stops = filelist.getFileList(path).Where(S => S[0] != 'P');
    ...
    public async Task Getter()
    {
        ...
        string EndData = $"{directory}EndDataP.txt";
    ...
    public void Sender(string CurrentStop, double WaitingTime, bool toEarly)
    {
        ...
        using (var writer = new StreamWriter(SystemData))
        {
            ...
            else
            {
                ...
                writer.WriteLine($"It's to early for this line, first train will come at {parameters.StartHour_H}:{parameters.StartHour_Min}0");
                writer.Flush();
            }
        }
    }
