...
[Parameters(0.1, 0.9, 0.1, 1.1, 2.3, 1.0, 1.0, 5, 00)]

//Wykorzystanie Interface - Punkt w Scorecard: Kontrakty: Interface
public class Metro : MeansOfTransport
{
    //Pobranie atrybutow dla klasy Bus - Punkt w Scorecard: Atrybuty własne
    Parameters parameters = (Parameters)Attribute.GetCustomAttribute(typeof(Metro), typeof(Parameters));
    private string meansOfTransport = "Metro";
    ...
    public async Task FileReader()
    {
        ...
        await Task.Run(() =>
        {
            ...
            IEnumerable<string> Lines = filelist.getFileList(path).Where(L => L[0] == 'M');
            IEnumerable<string> Stops = filelist.getFileList(path).Where(S => S[0] != 'M');
   ...
   public void CalculationOfDelay()
    ...
    public async Task Getter()
    {
        ...
        string EndData = $"{directory}EndDataM.txt";
    ...
    public void Sender(string CurrentStop, double WaitingTime, bool toEarly)
    {
        ...
        using (var writer = new StreamWriter(SystemData))
        {
            ...
            else
            {
                ...
                writer.WriteLine($"It's to early for this line, first metro will come at {parameters.StartHour_H}:{parameters.StartHour_Min}0");
                writer.Flush();
            }
        }
    }
